#!/usr/bin/env python3
"""GAM DeepAgents Core Logic v3.

Enhanced business logic for GAM DeepAgents operations with:
- Enhanced error handling and retry logic (v2)
- Programmatic tool calling capabilities (v2)
- Token-efficient patterns (v2)
- Improved observability (v2)
- Better separation of concerns (v2)
- KISS performance improvements (v3)
- Docker integration for MLflow, MindsDB, Redis, PostgreSQL (v3)
- Enhanced code generation and workflow orchestration (v3)
...
from scripts.gam_deepagents.v3.settings import GAMDeepAgentsSettingsV3, get_settings_v3
...
# Initialize logger
logger = logging.getLogger("gam-deepagents-core-v3")
...
def _estimate_tokens(text: str) -> int:
    ...
    """
    try:
        from scripts.gam_deepagents.shared.token_counter import TokenCounter
...
class GAMDeepAgentsCoreV3:
    """Core GAM DeepAgents business logic v3.

    Enhanced version with:
    - Better error handling and retry logic (v2)
    - Programmatic tool calling support (v2)
    - Token-efficient patterns (v2)
    ...
    def __init__(self, settings: GAMDeepAgentsSettingsV3 | None = None):
        """Initialize core with settings.

        Args:
            settings: Optional settings (defaults to get_settings_v3())
        """
        self.settings = settings or get_settings_v3()
        self.tracer = (
            get_opentelemetry_tracer("gam-deepagents-core-v3") if get_opentelemetry_tracer else None
        )
