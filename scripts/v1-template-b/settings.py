#!/usr/bin/env python3
"""Type-safe configuration for GAM DeepAgents MCP server v3 using pydantic-settings.
...
v3 enhancements:
- Docker service integration (MLflow, MindsDB, Redis, PostgreSQL)
...
class GAMDeepAgentsSettingsV3(BaseSettings):
    """Type-safe configuration for GAM DeepAgents MCP server v3.
    ...
    # Docker service configuration (v3)
    ...
    # Code generation settings (v3)
    code_generation_enabled: bool = Field(
        default=True,
        description="Enable enhanced code generation features (default: true)",
    )
    workflow_orchestration_enabled: bool = Field(
        default=True,
        description="Enable workflow orchestration features (default: true)",
    )
...
_settings_v3: GAMDeepAgentsSettingsV3 | None = None


def get_settings_v3() -> GAMDeepAgentsSettingsV3:
    """Get global settings instance v3 (singleton pattern).

    Returns:
        GAMDeepAgentsSettingsV3 instance with validated configuration
    """
    global _settings_v3
    if _settings_v3 is None:
        _settings_v3 = GAMDeepAgentsSettingsV3()
    return _settings_v3
