...
class RumbleTrack:
    ...
    def _add_hybrid_reverb(self):
        ...
        result = self.mcp.load_device(
            track_name=self.track_name, device_name="Hybrid Reverb"
        )
        if result.success:
    ...
    def _add_roar_saturation(self):
        ...
        result = self.mcp.load_device(track_name=self.track_name, device_name="Roar")
        if result.success:
    ...
    def _add_sidechain_compressor(self):
        ...
        result = self.mcp.load_device(
            track_name=self.track_name, device_name="Compressor"
        )
        if result.success:
