...
def test_analyze_alchemy_track_json():
    ...
    result = subprocess.run(
        ["uv", "run", str(ANALYSIS_SCRIPT), str(ALCHEMY_TRACK), "--json"],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def test_analyze_toxic_track_json():
    ...
    result = subprocess.run(
        ["uv", "run", str(ANALYSIS_SCRIPT), str(TOXIC_TRACK), "--json"],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def test_analyze_human_readable_output():
    ...
    result = subprocess.run(
        ["uv", "run", str(ANALYSIS_SCRIPT), str(ALCHEMY_TRACK)],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def test_analyze_nonexistent_file():
    ...
    result = subprocess.run(
        ["uv", "run", str(ANALYSIS_SCRIPT), nonexistent, "--json"],
        capture_output=True,
        text=True,
        timeout=10,
    )
...
def test_full_track_analysis():
    ...
    result = subprocess.run(
        ["uv", "run", str(ANALYSIS_SCRIPT), str(ALCHEMY_TRACK), "--full", "--json"],
        capture_output=True,
        text=True,
        timeout=60,  # Longer timeout for full track
    )
